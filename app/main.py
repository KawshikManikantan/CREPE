...
def model_init_():
    ...
    tokenizer = CanineTokenizer.from_pretrained('./model/model_save_epoch3/', do_lower_case=True)
    num_labels = 5
    batch_size = 32
    model = CanineForSequenceClassification.from_pretrained(
        "./model/model_save_epoch3/", # Use the 12-layer BERT model, with an uncased vocab.
...
PAGE_CONFIG = {'page_title':'SMAI Project','layout':"wide"}
...
if __name__ == '__main__':
    ...
    st.title("Rate the review")
    st.write('\n'*12)

    entered_text = st.text_area(label='Enter required text in the text area below, which would be used to predict the rating ')
    ...
    if submit:
        ...
        st.markdown(f"### Rating: {predicted_label}")